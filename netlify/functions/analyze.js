// Netlify Function - GMV日报生成器后端代理
const DASHSCOPE_API_KEY = process.env.DASHSCOPE_API_KEY || 'sk-ws-H.RYMLDHE.whau.MEYCIQDquxZlik8DU5m2K0mSSHGKcwkdJPYVXjgO98gGtq3H-QIhAIPwVswRmUE9XKvHpTOzM6LmgqD21SL69VlG5HPH8MLw';
...
async function callDashScope(prompt, image) {
  const body = {
    model: 'qwen-vl-max',
    messages: [{
      role: 'user',
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const gmv_image = payload.gmv_image;
    const top10_image = payload.top10_image;

    let report = '';

    if (gmv_image) {
      report = await callDashScope(GMV_PROMPT, gmv_image);
    }

    if (top10_image) {
      const t10 = await callDashScope(TOP10_PROMPT, top10_image);
      if (report) {
        const idx = report.indexOf('今日top10商品：');
        if (idx !== -1) {
          report = report.substring(0, idx) + t10;
        } else {
          report += '\n\n' + t10;
        }
      } else {
